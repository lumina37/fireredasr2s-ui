...
def uploadfile():
    try:
        ...
        print(f'{model=}')
        if not Path(f'{ROOT_DIR}/pretrained_models/FireRedASR2-{model}-L/model.pth.tar').exists():
            return jsonify({"code": 2, 'error': f'请下载 {model} 模型并放入 {ROOT_DIR}/pretrained_models/FireRedASR2-{model}-L/'}),500
...
def asr_task(wavs,asr_type='AED'):
    global _MODEL_CACHE
    if asr_type not in _MODEL_CACHE:
        cfg = FireRedAsr2Config(
            ...
            eos_penalty=1.0,
            decode_min_len=0,
            # FireRedASR2-LLM: repetition_penalty 3.0 / llm_length_penalty 1.0
            # avoid degeneration (official FireRedASR recommendation).
            repetition_penalty=3.0,
            llm_length_penalty=1.0,
            temperature=1.0,
        )
        _MODEL_CACHE[asr_type] = FireRedAsr2.from_pretrained(
            asr_type.lower(),
            f'{ROOT_DIR}/pretrained_models/FireRedASR2-{asr_type}-L',
            cfg)
    model = _MODEL_CACHE[asr_type]
